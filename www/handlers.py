...
import time,re,logging,hashlib,base64,asyncio,json

from config import configs
from models import User,Blog,Comment,next_id,Category

from coroweb import get,post
from apis import APIValueError,APIResourceNotFound,APIError,APIPermissionError,Page


COOKIE_NAME = 'awesession'
_COOKIE_KEY = configs.sessions.secret
...
@post('/api/test')
def api_test(request,*,name,summary,content):
    # check_admin(request)
    print(request)
    print(name)
    print(summary)
...
def text2html(text):
    lines = map(
        lambda s:'<p>%s</p>'
                 %s.replace('&','&amp')
                     .replace('<','&lt')
                     .replace('>','&gt'),filter(lambda s:s.strip() != '',text.split('\n')))
    return ''.join(lines)
...
@get('/')
async def index(*,page='1'): #缺少参数request
